...
Iron-based shape memory alloys (Fe-SMAs) are a promising low-cost alternative to Nitinol for civil-infrastructure and biomedical applications, but their pseudoelastic response is sensitive to composition and processing in ways that remain difficult to predict in advance. Large language model (LLM) deep-research agents, coupled with thermodynamic-stability databases, can now propose candidate compositions in seconds, which raises the practical question of whether such proposals survive physical synthesis. Here we report what may be the first synthesis and mechanical characterization of a novel Fe-Mn-Al-Si-Ni-C composition hypothesized by an LLM deep-research agent, benchmarked against the established Omori Fe-Mn-Al-Ni alloy processed under identical conditions; both compositions were verified by ICP-AES. The AI-hypothesized alloy (measured Ni 4.81 wt%, roughly half the 8.94 wt% of the benchmark) was readily castable but, on unloading from 2% applied strain, recovered only ≈0.5% strain — a value consistent with elastic springback alone, with no transformation plateau — across all four heat treatments examined. The benchmark recovered up to ≈1.1% under the same conditions, with a stress plateau characteristic of stress-induced α↔γ transformation. Synchrotron X-ray diffraction at the Shanghai Synchrotron Radiation Facility indicated that the AI alloy was a dual-phase mixture of γ-FCC austenite and an ordered B2/DO₃ Fe-Al phase rather than the single transforming matrix that pseudoelasticity requires, and its pattern was essentially unchanged by 10% applied strain. We attribute the absence of pseudoelasticity primarily to insufficient nickel — too little to form the targeted β-NiAl, so an ordered Fe₃Al phase formed instead. These observations suggest that compositions proposed by this kind of LLM-agent route may benefit from explicit pre-synthesis screening of competing phases. A secondary, more tentative observation from the benchmark alloy is that higher-feed-rate continuous strand annealing may yield refined-grain microstructures with improved pseudoelastic recovery; this would warrant systematic study.
...
**Table 1.** Measured chemical compositions (ICP-AES; C and O by ASTM E1019) of the AI-hypothesized alloy (heat RD0697-6) and the Omori benchmark (heat RD0697-7), in wt%. "n.r.": not reported in that analysis.

| Element | AI alloy (RD0697-6) | Omori benchmark (RD0697-7) |
|---------|---------------------|----------------------------|
...
Co, Ta, Ti, Cr, and Mo were each below 0.01 wt% in the benchmark, confirming the cost-driven exclusion of cobalt and tantalum. The most consequential difference between the two alloys is nickel: the AI composition contains 4.81 wt% Ni, roughly half the 8.94 wt% of the working benchmark (Sec. 4.1).
...
| Strand anneal, 0.8 ft/min | Continuous strand furnace, 1200 °C | ≈5 min | AI, Omori |
| Strand anneal, 4 ft/min | Continuous strand furnace, 1200 °C | ≈1 min | AI, Omori |
| Strand anneal, 20 ft/min | Continuous strand furnace, 1200 °C | ≈12 s | AI, Omori |

The 3-cycle AGG condition follows the abnormal-grain-growth protocol of Omori *et al.* [@omori2011superelastic], designed to produce bamboo-like single-crystal-diameter grains. The strand-anneal conditions span low-to-high continuous feed rates (0.8, 4, and 20 ft/min) and are intended to probe the range over which conventional industrial throughputs might replace the slow batch AGG cycle. For the ≈4 ft continuous hot zone used here, these feed rates correspond to hot-zone residence times of roughly 5 min (0.8 ft/min), 1 min (4 ft/min), and 12 s (20 ft/min); the "5 min" and "12 s" labels used in **Fig. 2** and Sec. 4.3 refer to the 0.8 and 20 ft/min conditions, respectively.
...
## 4.1 Why the AI alloy may not have transformed: nickel starvation

The most likely explanation is a nickel deficit. The AI agent targeted strengthening by β-NiAl (B2) precipitation together with carbides, but the measured Ni content of the AI alloy is only 4.81 wt%, against 8.94 wt% in the working Omori benchmark (Table 1) and the ≈7.5–8.5 wt% Ni typical of superelastic Fe-Mn-Al-Ni [@omori2011superelastic; @omori2017martensitic; @abuzaid2019femnnial]. At roughly half the benchmark nickel, there is insufficient Ni to stabilize a β-NiAl-strengthened, transformable matrix. Instead the available aluminium partitions into an ordered Fe₃Al (B2/DO₃) phase, producing the dual-phase γ-FCC + Fe₃Al structure seen in every condition (Sec. 3.1) and in the diffraction patterns (Sec. 3.3). In short, the targeted β-NiAl and carbide constituents did not form; the alloy is dominated instead by ordered Fe-Al. This is the single factor most directly supported by the measured chemistry.

A second, more tentative factor is the carbon content (measured 0.105 wt%, against 0.010 wt% in the benchmark). We hypothesize that this carbon partitions into, and helps stabilize, the ordered Fe-Al/ferritic constituent rather than the FCC austenite, reinforcing the dual-phase tendency [@bhadeshia2010steels]. We label this explicitly as a hypothesis: confirming it would require CALPHAD modelling of the Fe-Mn-Al-Ni-Si-C system and quantitative carbon partitioning measurements, neither of which was performed here, and we do not invoke a specific interstitial-ordering mechanism on the present evidence. (Magnetic effects during high-temperature processing may also play a minor role but are not constrained by our data.)
...
We are cautious about generalizing from a single composition, and we restrict the following to the specific LLM-deep-research route used here rather than to AI-guided design in general. For that route, the failure mode we observed — a targeted precipitate (β-NiAl) that never formed because the proposed nickel level was too low, leaving an ordered Fe-Al phase to dominate — could plausibly have been caught before any metal was melted. We suggest a short, deterministic screening loop to run on the agent's output prior to synthesis:
...
2. **Stoichiometry check.** Verify that the proposed level of the key precipitate-forming element (here, Ni for β-NiAl) is sufficient relative to established working alloys, flagging proposals that fall well below the literature range.
...
4. **Processing-route check.** Confirm that the intended anneal/feed-rate schedule can plausibly reach the target microstructure.
...
As a secondary observation (Sec. 3.2, Table 3), the 1200 °C / 12 s (20 ft/min) strand-anneal condition on the Omori benchmark recovered ≈1.1% strain at the 2% reference cycle, against ≈0.5% for the 1200 °C / 5 min (0.8 ft/min) condition on the same chemistry — roughly a factor of two, with only the faster condition exceeding the elastic strain. This is consistent with the possibility that, within a bounded processing window, sufficiently refined grains embedded in a coherent matrix may aid favorable variant selection, and that faster strand-anneal conditions may retain a finer, more shearable precipitate dispersion. We emphasize that this rests on single specimens (n = 1 per condition) and three feed rates and should be regarded as preliminary; it is not, on its own, evidence against the role of large grains established in the Fe-Mn-Al-Ni literature [@omori2011superelastic; @vollmer2016cyclic]. A systematic feed-rate study with replicates would be needed to assess whether it reflects a reproducible processing window.
...
Fourth, the processing observation in Sec. 4.3 rests on a limited set of feed rates (0.8, 4, and 20 ft/min) and single specimens per condition; it should not be elevated from observation to design rule without a systematic study. Electron backscatter diffraction (EBSD) was also not performed, and would directly resolve variant selection and grain-boundary character in the refined-grain Omori specimens.
...
1. The AI-hypothesized alloy was readily castable but, on unloading from 2% applied strain, recovered only ≈0.5% — consistent with elastic springback alone and with no transformation plateau — across all four heat treatments examined, including loading to 8–10% strain. Its cyclic response is consistent with deformation by dislocation slip rather than stress-induced martensitic transformation.
2. The absence of pseudoelasticity is attributed primarily to nickel starvation: at a measured 4.81 wt% Ni (roughly half the 8.94 wt% of the working benchmark), the alloy contained too little nickel to form the targeted β-NiAl, and an ordered Fe₃Al (B2/DO₃) phase formed instead. Synchrotron XRD confirms the resulting dual-phase γ-FCC + Fe₃Al structure and shows its pattern essentially unchanged by 10% applied strain. A possible secondary contribution from the alloy's higher carbon content is noted as a hypothesis pending CALPHAD confirmation.
3. As a secondary, preliminary observation, refined-grain (faster strand-anneal) processing of the benchmark recovered roughly twice the strain of the slower condition (≈1.1% vs ≈0.5% at 2% applied); this would warrant systematic investigation before any design conclusion is drawn.
