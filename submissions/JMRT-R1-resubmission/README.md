...
| `figures/Figure_1.jpg` … `Figure_9.png` (9 files) | **Figure** (one item per figure, in order) | Packaged 2026-08-21. Filename = figure number, so `Figure_4.jpg` is Fig. 4 in the text and caption list. Each file is **byte-identical (SHA-256) to the image embedded in the clean DOCX** — the separate uploads and the manuscript cannot drift. Per-file pixel size, stored dpi, effective dpi at 90/190 mm column width, hashes and the manuscript captions are in `figures/UPLOAD-MANIFEST.md`. |
| `figures/Table_1.docx`, `Table_2.docx`, `Table_3.docx` | **Table** (one item per table) — *only if the portal asks for tables separately* | Added 2026-08-21. Editable Word tables, not images. **All three are already inline in the manuscript DOCX**, which is where Elsevier normally wants them — upload these only if the portal has a separate Table item type. Table 3 is **new in this revision** (CALPHAD equilibrium phase fractions, answering R1 #1 and #2) and Table 2 is **revised** (n = 3, mean ± SD); Table 1 is unchanged. Each was verified cell-by-cell against the corresponding table in the clean DOCX and opens in Word with no repair prompt. |
| `Cai_Fe-SMA_JMRT_R1_figures-and-tables.zip` | *convenience only* | All twelve files zipped (4.20 MB), if the portal accepts a single archive. Prefer the individual uploads. |
